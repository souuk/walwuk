...
const minimumPriorMs = Math.max(0, Number.parseInt(option("minimum-prior-ms", "75"), 10));
...
async function dynamicSearch(fixture, prior, expected, predictionStable) {
  ...
  // Establish one exact incumbent before distributing alternatives. The v6
  // speculative prescreen duplicated work whenever its provisional bound was
  // too weak; seed-first scheduling trades a short synchronization point for
  // a trustworthy alpha shared by every subsequent root task.
  const seed = await workers[0].search(
    [predictedMove], -INFINITY, {fullWindow: true},
  );
  nodes += seed.nodes;
  researches += seed.researches;
  let incumbent = {move: predictedMove, result: seed.results[0].result};
  const queue = alternatives;
  let queueIndex = 0;
...
console.log(JSON.stringify({
  scheduling: "persistent-ready-seed-first-dynamic-root-v7",
  depth,
  workerCount,
  minimumPriorMs,
  batchSize,
